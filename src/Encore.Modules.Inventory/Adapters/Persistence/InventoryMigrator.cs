...
/// <summary>
/// Applies Inventory's outstanding migrations while the host is starting, so a
...
/// when <c>Inventory:MigrateOnStartup</c> is set, which it is in the run
...
/// </para>
/// </remarks>
public sealed class InventoryMigrator(
    IServiceScopeFactory scopeFactory,
    ILogger<InventoryMigrator> logger) : IHostedLifecycleService
{
    private readonly IServiceScopeFactory _scopeFactory = scopeFactory;
    private readonly ILogger<InventoryMigrator> _logger = logger;
    ...
    public async Task StartingAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation("Inventory schema is up to date.");
            return;
        }

        _logger.LogInformation(
            "Applying {Count} Inventory migration(s): {Migrations}.",
            pending.Count,
            string.Join(", ", pending));

        await context.Database.MigrateAsync(cancellationToken).ConfigureAwait(false);

        _logger.LogInformation("Inventory schema updated.");
    }
