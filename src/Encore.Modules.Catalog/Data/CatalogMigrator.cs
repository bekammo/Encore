...
/// <summary>
/// Applies Catalog's outstanding migrations while the host is starting, so a
...
/// <c>Catalog:MigrateOnStartup</c> is set, which the run profiles do and nothing
...
/// </para>
/// </remarks>
public sealed class CatalogMigrator(
    IServiceScopeFactory scopeFactory,
    ILogger<CatalogMigrator> logger) : IHostedLifecycleService
{
    private readonly IServiceScopeFactory _scopeFactory = scopeFactory;
    private readonly ILogger<CatalogMigrator> _logger = logger;
    ...
    public async Task StartingAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation("Catalog schema is up to date.");
            return;
        }

        _logger.LogInformation(
            "Applying {Count} Catalog migration(s): {Migrations}.",
            pending.Count,
            string.Join(", ", pending));

        await context.Database.MigrateAsync(cancellationToken).ConfigureAwait(false);

        _logger.LogInformation("Catalog schema updated.");
    }
