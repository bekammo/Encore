...
/// <summary>
/// Applies Orders' outstanding migrations while the host is starting, so a
...
/// <c>Orders:MigrateOnStartup</c> is set, which the run profiles do and nothing
...
/// </para>
/// </remarks>
public sealed class OrdersMigrator(
    IServiceScopeFactory scopeFactory,
    ILogger<OrdersMigrator> logger) : IHostedLifecycleService
{
    private readonly IServiceScopeFactory _scopeFactory = scopeFactory;
    private readonly ILogger<OrdersMigrator> _logger = logger;
    ...
    public async Task StartingAsync(CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation("Orders schema is up to date.");
            return;
        }

        _logger.LogInformation(
            "Applying {Count} Orders migration(s): {Migrations}.",
            pending.Count,
            string.Join(", ", pending));

        await context.Database.MigrateAsync(cancellationToken).ConfigureAwait(false);

        _logger.LogInformation("Orders schema updated.");
    }
